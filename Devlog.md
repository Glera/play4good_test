...
## #153 — Идеальная egg-curve анимация удаления пар костяшек
**Дата:** 2026-02-19
**Ветка:** dev/Gleb
**Финальный коммит:** `f82c79d`
**Статус:** ✅ готово к переносу

### Что сделано
Параметрическая egg-curve анимация удаления пар (8 итераций):
- Линейное продольное движение (ровно 50% расстояния в каждой фазе)
- Горизонтальный scatter (perpY=0) — кости ВСЕГДА разлетаются в стороны
- Кости всегда встречаются боковыми гранями на одной высоте
- Динамическая амплитуда (близкие и вертикально стоящие кости — больше разлёт)
- Квадратичное затухание латерали в последних 15% для чистого контакта
- Раннее удаление DOM-элементов на 94% прогресса

### Изменённые файлы
- `app.js` — animateArc(), removePair(), calcDirection()

### Как перенести в main (squash 8 коммитов в один)
```bash
git cherry-pick --no-commit 4825141^..f82c79d && git commit -m "feat: egg-curve анимация удаления пар (#153)"
```
