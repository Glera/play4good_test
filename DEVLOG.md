...
## #102 — сейчас когда выбирается первая кость пары она становится голубой, а вторая остае
**Дата:** 2026-02-06  
**Ветка:** dev/Gleb  
**Коммит:** `92ed60d`  
...
```bash
git cherry-pick 92ed60d1a4b2dafb94708761e3dc5448a47bcfb6
```

---


## #102 — вторая кость пары тоже становится голубой перед полётом
**Дата:** 2026-02-06
**Ветка:** dev/Gleb
**Коммит:** `264235e`
**Статус:** ✅ готово к переносу
...
### Что сделано
При выборе пары костей вторая кость теперь тоже подсвечивается голубым (класс `selected`) перед началом анимации полёта. Раньше только первая кость была голубой, а вторая оставалась коричневой.

### Изменённые файлы
- `app.js` — добавлено присвоение класса `selected` обоим элементам пары перед вызовом `animateArc`

### Как перенести в main
```bash
git cherry-pick 264235edc2853eeb38db70132123901f06cd0368
```

---

## #101 — давай делать доступные освобожденные кости в моент начала полета убранных костей
**Дата:** 2026-02-06  
**Ветка:** dev/Gleb  
**Коммит:** `55c390d`  
**Статус:** ✅ готово к переносу
...
### Как перенести в main
```bash
git cherry-pick 55c390d66e8069b3d8d9ca3fbc7f32641af6d8df
```

---


## #101 — делать доступные освобожденные кости в момент начала полета убранных костей
**Дата:** 2026-02-06
**Ветка:** dev/Gleb
**Коммит:** `aee67de`
**Статус:** ✅ готово к переносу
...
### Что сделано
Перенесён вызов updateTileStates() в функции removePair() с момента завершения анимации полёта на момент начала анимации. Теперь кости, которые были заблокированы убранной парой, становятся кликабельными сразу при начале анимации полёта, а не после её окончания (450мс).

### Изменённые файлы
- `app.js` — вызов updateTileStates() перенесён до начала animateArc(), убран дублирующий вызов из onFinish()

### Как перенести в main
git cherry-pick aee67de58f54f2db508930889b0bc11f4ce630ca

---

## #100 — пускай заблокированная кость слегка пошатается из стороны в сторону по тапу
**Дата:** 2026-02-06  
**Ветка:** dev/Gleb  
**Коммит:** `588472b`  
**Статус:** ✅ готово к переносу
...
### Как перенести в main
```bash
git cherry-pick 588472ba67cae1eeda7cb04198f9084b7b3b8bc9
```
...
### Что сделано
Добавлена анимация покачивания (shake) при тапе на заблокированную кость. Когда игрок нажимает на кость, которая не является свободной, она слегка покачивается из стороны в сторону, давая визуальную обратную связь о невозможности выбора.

### Изменённые файлы
- `app.js` — добавлена обработка тапа по заблокированным костям в onTileClick(): при тапе добавляется CSS-класс shake с перезапуском анимации
- `style.css` — добавлена CSS-анимация tile-shake (0.4s, translateX ±3px с лёгким поворотом)

### Как перенести в main
```bash
git cherry-pick 4f7d32def12d8b9f8f721e2b6c212fa9e0367516
```
